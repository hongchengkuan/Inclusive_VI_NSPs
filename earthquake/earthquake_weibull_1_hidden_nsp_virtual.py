...
if __name__ == "__main__":
    ...
    opt.update_batch_size = 209
    opt.model_opt_flag = False
    opt.eval_batch_size = 209
    opt.time_scale = 1e-3
    ...
    DPPs.add_PP(
        ...
        kernel_params_opt=True,
        base_rate_opt=True,
        virtual_background_rate=torch.tensor([1.6557e-7]),
    )
    ...
    var_opt = torch.optim.Adam(get_variational_params(DPPs), 1, eps=1e-06)
    model_opt = torch.optim.Adam(get_model_params(DPPs), 1, eps=1e-06)
