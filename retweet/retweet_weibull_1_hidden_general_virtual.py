...
if __name__ == "__main__":
    ...
    opt.train_obs_size = 20000
    opt.dev_obs_size = 200
    opt.test_obs_size = 2000
    opt.batch_size = 200
    opt.training_data_ratio = 1
    opt.dev_data_ratio = 1
    opt.test_data_ratio = 1
    opt.update_batch_size = 200
    opt.model_opt_flag = True
    opt.eval_batch_size = 1000
    opt.time_scale = 1e-3
    opt.init_tot_burn_in_steps = 400
    opt.tot_burn_in_steps = 100
    opt.burn_in_steps = 1
    ...
    DPPs.add_PP(
        ...
        top=True,
        bottom=False,
        virtual_kernel_params=torch.tensor([[1 / 2.5, 1.0, 1.5], [1 / 2.0, 1.0, 3.2], [1.0 / 0.2, 1.0, 7.93]])[None, None, ...],
        virtual_prop_background_rate=0.05,
        dropout=0.1,
        kernel_params_opt=True,
        base_rate_opt=True,
        virtual_background_rate=torch.tensor([0.05]),
    )
    ...
    var_opt = torch.optim.Adam(get_variational_params(DPPs, nn_lr=1e-3), lr=1, eps=1e-06)
